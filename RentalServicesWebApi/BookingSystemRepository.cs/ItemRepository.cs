    ...
    public class ItemRepository : GenericRepository<Item>, IItemRepository
    {
        ...
        public override async Task<bool> Update(Item item)
        {
            try
            {
                ...
                existItem.Availability = item.Availability;
                existItem.BoughtDate = item.BoughtDate;
                existItem.Brand = item.Brand;
                existItem.ItemCondition = item.ItemCondition;
        ...
        public override async Task<Item> GetByName(string name)
        {
            try
            {
                return await dbSet.Include(p => p.Category).FirstOrDefaultAsync(d => d.Name == name);
            }
            catch (Exception e)
            {
                _logger.LogError(e, "{Repo} All function error", typeof(ItemRepository));
        ...
        public override async Task<IEnumerable<Item>> FindByName(string name)
        {
            try
            {
                return await dbSet.Include(c=> c.Category).Where(x => x.Name == name).ToListAsync();
